...
> **How this maps to this skill's L1–L4 stack:** the "Cited" rung below ≈ **L4 (Informational Citation)** — the layer this skill measures with `seo_geo_score` / `ai_visibility_score`. The "Recommended" rung ≈ **L3 (Category Citation)** — the off-tool layer this skill flags as requiring off-site investment. This reference is the "why" behind that off-tool label: recommendation is governed by web-wide consensus, not by your own pages. Use `entity_mentions` for a proxy read on the off-site footprint (L2/L3) that feeds it.
...
| **2. Cited** (≈ L4) | Your page appears as a source in the answer | Content usefulness: structure, statistics, clarity, freshness | Prompt-tracking tools, AI Overview source lists |
| **3. Mentioned** | Your brand is named in the answer text | Entity recognition + how the web talks about you | Prompt-tracking tools, `entity_mentions` |
| **4. Recommended** (≈ L3) | Your product is on the shortlist the buyer actually considers | **Aggregate web consensus** — reviews, forums, analysts, press, video — largely independent of your own content | Prompt tracking + the framing around the mention |
...
Recommendation is a consensus signal. The inputs the models weigh live mostly off your site (all of these are **off-tool** — outside what the thatseoagent MCP can change directly; `entity_mentions` gives a proxy read on how much footprint exists):

| Channel | Why it moves recommendations |
|---|---|
| **Review platforms** (G2, Capterra, TrustRadius, app stores) | Third-party validation models treat as evidence of legitimacy |
| **Analyst coverage** (Gartner, Forrester, industry reports) | High-authority category framing; models echo analyst shortlists |
| **Communities and forums** (Reddit, HN, Slack/Discord, niche forums) | Unprompted practitioner discussion is heavily retrieved and hard to fake |
| **Earned media and PR** | Independent sources repeating your positioning beyond your own site |
| **Video and podcasts** | Increasingly retrieved; transcripts carry brand + category associations |
...
**The attribution blind spot:** in the SimilarWeb data, only about **9%** of those post-recommendation visits arrived as visible AI referral traffic; the largest share arrived via branded search, with direct and other channels making up the rest — indistinguishable from ordinary organic visitors. AI recommendations are already sending real, engaged buyers, but standard attribution underreports the AI touch. (`ga4_ai_traffic` reads that visible slice off GA4's own "AI Assistant" channel — Google assigns `medium = ai-assistant` when it recognises the referrer. Treat it as a floor, not the full picture: it can only see visits where the assistant passed a referrer at all.)

**Measurement triad** (no single signal is complete; together they give a reliable read):

1. **AI prompt tracking** — whether and how you're mentioned/recommended in LLM answers, even when no click ever lands. Track the framing around mentions — recommended, neutral, hedged, or recommended-against — not just the count.
...
Also watch **branded search volume** (`gsc_branded_split`) as a proxy: sustained lifts without a matching campaign are increasingly AI-influence showing up under another name.
