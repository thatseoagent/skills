...
Detailed evidence backing the Hreflang Validation section of the Technical SEO skill. Organized by topic with source URLs and key quotes. The `seo_hreflang_validator` tool checks the mechanics (self-reference, reciprocity, valid codes, x-default, reachability); this reference is the "why" and the at-scale guidance the tool cannot give.
